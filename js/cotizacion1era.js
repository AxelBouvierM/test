...
function crearCoti() {
    let botType = window.localStorage.getItem('botType')
    let cub = +document.querySelector('#inputCUB').value
    cui = +document.querySelector('#inputCUI').value
    ...
    // Verifica si ImageBot, Voice2Text y Catálogo están seleccionados
    ...
    if (soporte[0] > 0) {
        ...
        coti.push({
            item: `Soporte básico`,
            descr: `- Soporte básico (${sesionesHey[0] == '2500' ? '4 horas' : '8 horas'} de dedicación)`,
            precio: +soporte[1],
            tipoPago: 'Pago mensual',
            horas: 0
        })
